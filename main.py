...
class DataGovernanceFramework:
    ...
    def assess_data_quality(self, df: pd.DataFrame, dataset_name: str = "unknown_dataset") -> Dict[str, Any]:
        ...
        print(f"[SEARCH] Starting data quality assessment for: {dataset_name}")
        ...
        print("\n[STEP 1] Identifying critical data elements...")
        ...
        print(f"   [CHECK] Critical fields identified: {critical_fields}")
        print(f"   [CHECK] Unique fields identified: {unique_fields}")

        # Step 2: Calculate data quality scores
        print("\n[STEP 2] Calculating data quality scores...")
        quality_results = self.scorer.calculate_overall_quality_score(df, field_profiles)

        print(f"   [CHECK] Overall dataset score: {quality_results['dataset_score']:.1f}% (Grade: {quality_results['dataset_grade']})")
        ...
        print(f"   [CHECK] Output directories created under: {PROCESSED_DIR / raw_file.stem}")
        ...
        print(f"   [CHECK] Metadata saved to: {metadata_filename}")

        # Step 5: Log to data lineage
        print("\n[STEP 5] Logging to data lineage...")
        ...
        print(f"   [CHECK] Lineage entry created: {lineage_entry['assessment_id']}")
        print(f"   [CHECK] Lineage saved to: {lineage_filename}")
    ...
    def generate_quality_report(self, assessment_results: Dict[str, Any]) -> str:
        ...
        # Overall Summary
        report.append("[DATA] OVERALL SUMMARY")
        report.append("-" * 40)
        ...
        report.append("[SEARCH] FIELD-BY-FIELD ANALYSIS")
        report.append("-" * 40)
        ...
        for field, scores in quality_results['field_scores'].items():
            ...
            # Anomalies
            if scores['contextual_anomalies']:
                report.append(f"   [WARNING] Anomalies Found: {scores['contextual_anomalies']}")
                if scores['ai_suggestions']:
                    report.append(f"   [AI] AI Suggestions: {scores['ai_suggestions']}")
        ...
        if low_quality_fields:
            report.append("[ALERT] Fields requiring immediate attention:")
        ...
        if critical_issues:
            report.append("\n[CRITICAL] Critical fields with quality issues:")
        ...
        print(f"[DOCUMENT] Quality report saved to: {report_filename}")
        return report_text
...
def create_test_data() -> pd.DataFrame:
    ...
    data = {
        ...
        'Item': np.random.choice(['Coffee', 'Tea', 'Cake', 'Cookie', 'Sandwich', 'ERROR', 'UNKNOWN'], n_records, p=[0.3, 0.2, 0.15, 0.15, 0.15, 0.03, 0.02]),
        'Quantity': np.random.choice([1, 2, 3, 4, 5, -1, 0, 999], n_records, p=[0.4, 0.3, 0.15, 0.08, 0.05, 0.01, 0.005, 0.005]),
        'Price_Per_Unit': np.round(np.random.uniform(1.0, 15.0, n_records), 2),
        'Total_Spent': np.round(np.random.uniform(1.0, 50.0, n_records), 2),
        'Transaction_Date': pd.date_range('2023-01-01', periods=n_records, freq='H'),
        'Payment_Method': np.random.choice(['Cash', 'Card', 'Mobile', None, 'ERROR'], n_records, p=[0.3, 0.4, 0.25, 0.03, 0.02]),
        'Location': np.random.choice(['Store_A', 'Store_B', None, 'UNKNOWN'], n_records, p=[0.4, 0.4, 0.15, 0.05])
    }

    # Introduce some duplicates in Transaction_ID (should be unique)
    duplicate_indices = np.random.choice(range(n_records), size=10, replace=False)
    for idx in duplicate_indices:
        data['Transaction_ID'][idx] = data['Transaction_ID'][0]  # Create duplicates

    # Introduce some missing values
    missing_indices = np.random.choice(range(n_records), size=50, replace=False)
    for idx in missing_indices:
        data['Payment_Method'][idx] = None

    # More missing values in Location
    missing_location_indices = np.random.choice(range(n_records), size=200, replace=False)
    for idx in missing_location_indices:
        data['Location'][idx] = None
...
def process_data_file(file_path: Path) -> None:
    ...
    print(f"\n[CHECK] Data Governance Framework Assessment Complete for {file_path.name}!")
    print("\nFiles created:")
...
def run_basic_test():
    """Run basic functionality test with generated test data"""
    print("[TEST] Running Basic Functionality Test...")
    print("=" * 60)

    # Create test data
    print("[DATA] Creating test dataset...")
    df = create_test_data()
    print(f"   [CHECK] Test dataset created: {df.shape}")

    # Initialize framework
    framework = DataGovernanceFramework()

    # Run assessment
    print("\n[SEARCH] Running data quality assessment...")
    assessment_results = framework.assess_data_quality(df, "test_dataset")

    # Generate report
    print("\n[DOCUMENT] Generating quality report...")
    report = framework.generate_quality_report(assessment_results)
    ...
    print("=" * 60)
    print(f"Overall Score: {quality_results['dataset_score']:.1f}% (Grade: {quality_results['dataset_grade']})")
    print(f"Critical Fields Average: {quality_results.get('critical_fields_avg', 0):.1f}%")

    # Show field scores
    print("\nField Scores:")
    for field, scores in quality_results['field_scores'].items():
        print(f"  {field}: {scores['overall_score']:.1f}% ({scores['grade']})")

    print("\n[CHECK] Basic functionality test completed successfully!")
    return True
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Data Governance Framework')
    ...
    if args.test:
        ...
        try:
            ...
            print("\n[PARTY] File processing complete!")
        except Exception as e:
            print(f"[ERROR] Error processing file: {e}")
            sys.exit(1)

    else:
        # Default behavior - process all files in data directory
        print("[ROCKET] Data Governance Framework Starting...")
        print(f"[FOLDER] Data directory: {DATA_DIR.resolve()}")
        print(f"[FOLDER] Processed directory: {PROCESSED_DIR.resolve()}")
        ...
        if not data_files:
            ...
            print("\nOr run with --test flag to test basic functionality:")
            print("  python main.py --test")
        else:
            print(f"\n[DATA] Found {len(data_files)} file(s) to process:")
